...
-   $$ \text{Spread} = \text{Interest rate by Company} - \text{Interest rate by US Treasury bond} $$
...
```{r}
# Forecast 30 days values of Corporate index bond

#last_var_corp <- tail(corp$value, 1)

#forecasted_values_var_corp <- numeric(length = #length(forecast_var$fcst$corp_value[, "fcst"]))

#for (i in 1:length(forecast_var$fcst$corp_value[, "fcst"])) {
#  forecasted_diff_var_corp <- forecast_var$fcst$corp_value[, "fcst"][i]
#  forecasted_values_var_corp[i] <- last_var_corp + #forecasted_diff_var_corp
#  last_var_corp <- forecasted_values_var_corp[i]
#}

#print(forecasted_values_var_corp)
```
